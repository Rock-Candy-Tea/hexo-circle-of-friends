...
def query_all(li, start: int = 0, end: int = 0, rule: str = "updated"):
    ...
    # 计算last_update_time
    last_update_time_results = list(post_collection.find({}, {"createdAt": 1, "_id": 0}).limit(1000))
    if last_update_time_results:
        last_update_time = max(doc.get("createdAt", "1970-01-01 00:00:00") for doc in last_update_time_results)
    else:
